...
- [2026-08-07 MCP result overflow continuation](docs/findings/2026-08-07-mcp-result-overflow-continuation.md) — oversized `tools/call` results split into chunks returned as multiple `content[]` text blocks (eager); `execute_tools` packages per `results[]` item (stubs + `resultIndex` + `partsIncluded`); `continue_tool_result` for deferred/re-fetch; `MAX_OVERFLOW_ITEMS` (default 2) caps eager oversized items per batch; `complete: false` when remainder omitted. **Part cap superseded by the 2026-08-13 token budget.**
- [2026-08-13 overflow token budget](docs/findings/2026-08-13-overflow-token-budget.md) — `MAX_OVERFLOW_PARTS` replaced by `MAX_OVERFLOW_TOKENS` (default 300000); `MAX_RESULT_CHARS` promoted to env (default 60000); part count = `MAX_OVERFLOW_TOKENS / MAX_RESULT_CHARS` so changing block size does not retune the budget
