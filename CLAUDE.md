...
- [2026-08-07 MCP result overflow continuation](docs/findings/2026-08-07-mcp-result-overflow-continuation.md) — oversized `tools/call` results split into chunks returned as multiple `content[]` text blocks (eager); `execute_tools` packages per `results[]` item (stubs + `resultIndex` + `partsIncluded`); `continue_tool_result` for deferred/re-fetch; `MAX_OVERFLOW_PARTS` (default 5) caps parts per payload; `MAX_OVERFLOW_ITEMS` (default 2) caps eager oversized items per batch; `complete: false` when remainder omitted
