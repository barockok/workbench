...
try {
  ...
  if (pageErrors.length) for (const e of pageErrors) console.error(`page error: ${e}`);
  process.exitCode = worst > 0.15 ? 1 : 0;
} catch (err) {
  console.error(`blip-check failed at step "${step}": ${err.message}`);
  process.exitCode = 1;
} finally {
  await browser.close();
}
