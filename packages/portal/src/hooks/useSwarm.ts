...
// entrance.
export function useSwarm(ref: RefObject<HTMLCanvasElement | null>, opts: { ground: "dark" | "accent"; enabled?: boolean; markX?: number | (() => number); ambient?: boolean }) {
  const swarm = useRef<Swarm | null>(null);
  const enabled = opts.enabled ?? true;
  useEffect(() => {
    if (!enabled || !ref.current) return;
    // markX may be a function so a caller can measure the canvas's actual
    // laid-out size right here, after paint, instead of during render (when
    // CSS dimensions aren't settled yet).
    const markX = typeof opts.markX === "function" ? opts.markX() : opts.markX;
    swarm.current = createSwarm(ref.current, { ground: opts.ground, markX, ambient: opts.ambient });
