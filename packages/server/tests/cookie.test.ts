...
describe("cookie auth", () => {
  ...
  describe("isCookieExpired", () => {
    ...
    it("returns false when one cookie expired but a live one remains (don't let junk poison the set)", () => {
      // Real ng-mis repro: a short-lived Keycloak/analytics cookie expires
      // seconds after capture, but the long-lived session token is still valid.
      const now = Math.floor(Date.now() / 1000);
      const data = {
        domain: "amartha.id",
        cookies: [
          { name: "KC_AUTH_SESSION_HASH", value: "x", domain: "sso.amartha.id", path: "/", expires: now - 10 },
          { name: "__Secure-next-auth.session-token.0", value: "y", domain: "ngmis.amartha.id", path: "/", expires: now + 86400 },
        ],
        capturedAt: now,
      };
      expect(isCookieExpired(data)).toBe(false);
