...
beforeEach(() => { db.exec("DELETE FROM oauth_clients"); db.exec("DELETE FROM pending_auth"); });
...
describe("/authorize", () => {
  it("302s to Google SSO and stores the pending request", async () => {
    ...
    const row = db.prepare("SELECT COUNT(*) AS n FROM pending_auth WHERE integration = '__oauth_authorize__'").get() as { n: number };
    expect(row.n).toBe(1);
  });
