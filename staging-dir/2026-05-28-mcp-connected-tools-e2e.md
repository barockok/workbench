...
**Date:** 2026-05-28
**Branch:** `staging/mcp-claude-sdk` (built on `2114e44` main + telemetry fix from `1bbb6bc`)
...
3. **API key minted for the SSO user.** Bypassing `createUser` (which inserts a new row) and updating the existing maria row directly:
   ...
   ```
4. **MCP server config (gitignored).** `staging-dir/.mcp.json` declares one HTTP MCP server:
   ```jsonc
...
| `staging-dir/.mcp.json` (Bearer = maria's API key) | `staging-dir/` | `staging-dir/.gitignore` line `.mcp.json` ✅ |
| Stream-json transcript (model output, no creds) | `staging-dir/mcp-connected.transcript.jsonl` | `*.transcript.jsonl` ignored ✅ |
...
| API key minted for maria | only in `staging-dir/.mcp.json` + local memory | not committed |
| Screenshot of connected dashboard | `staging-dir/dashboard-httpbin-connected.png` | contains email `<tester>@<workspace>` in the user-block — this IS committed; flag if you want it redacted before any future push |
...
# 5. Wire staging-dir/.mcp.json with that API key (gitignored).
