...
| atlassian-confluence| PASS — site picker → `example-confluence.atlassian.net` | `confluence_search_pages` `{query:"staging"}`         | PASS — real Confluence page `staging` (`id: 1234567890`) |
...
After the fix the same `jira_project_types` call returned a real JSON array of project types; `confluence_search_pages query=staging` returned a real page row from the Confluence space.
...
- **`jira_search_issues` calls a deprecated endpoint.** Atlassian removed `/rest/api/3/search` (CHANGE-2046). Plugin needs to switch to `/rest/api/3/search/jql`. Same fix shape for any other `/search` callers — out of scope for this staging.
...
| Tool                  | Result |
|-----------------------|--------|
| `confluence_search_pages` | PASS — real page `staging` (`id: 1234567890`) |
