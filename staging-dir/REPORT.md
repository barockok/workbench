...
### Gmail (Connected)
```
google_gmail_profile -> {emailAddress: "[redacted]@acme.com", messagesTotal: 32159, threadsTotal: 22106}
...
All secrets in `staging-dir/.env` (gitignored, never committed).
...
cd packages/server && npx tsx -e "import { createUser } from './src/auth/users.ts'; console.log(createUser('staging-test-user').apiKey);"
...
| `staging-dir/docker-compose.yml` | HTTP-only, port 3000 (new) |
| `staging-dir/staging-REPORT.md` | This report (new) |

---

## Test Artifacts

- **Database:** `staging-dir/data/tokens.db`
- **Test user:** `staging-test-user` (email: `[redacted]@acme.com`) with API key `[REDACTED]`
- **Container:** `staging-dir-a-workbench-1`
