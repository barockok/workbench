...
**Date:** 2026-05-28
**Branch:** main (post-merge of `docs/uat-report` → `2114e44`)
...
3. UAT API key minted via:
   ```
   tsx --env-file=../../.env -e "const {createUser}=require('./src/auth/users.ts');console.log(JSON.stringify(createUser('uat-mcp-tester')))"
   ```
   Key stored in `uat-dir/.mcp.json` (gitignored, never committed).
4. MCP server config at `uat-dir/.mcp.json`:
   ```jsonc
   {
     "mcpServers": {
       "a-workbench": {
...
Returned 14 integrations, all with `"connected": false` (fresh UAT user).
...
### Notes
- This UAT only covers the MCP **protocol** path. End-to-end `execute_tool` against a real plugin (e.g. Gmail) still requires per-plugin OAuth creds in `.env` and a portal-driven Connect — that's covered by the earlier UAT-REPORT.md, not re-run here.
...
# 2. mint UAT user
tsx --env-file=../../.env -e "console.log(JSON.stringify(require('./src/auth/users.ts').createUser('uat-mcp-tester')))"

# 3. wire MCP config (uat-dir/.mcp.json — gitignored)
...
- `uat-dir/.mcp.json`: gitignored (added to `uat-dir/.gitignore` in this UAT)
- `uat-dir/claude-sdk.transcript.jsonl`: gitignored (`*.transcript.jsonl`)
