...
| 3 | google-sheets       | PASS    | `google_sheets_search` query=`UAT` → `{files: []}` | OK |
...
| 8 | atlassian-bitbucket | PASS    | `bitbucket_list_repos` workspace=`acme` → real repo list (e.g. `acme/internal-app`) | own OAuth app, not in dev mode |
...
| – | github              | BLOCKED | n/a | No `GITHUB_CLIENT_ID` / `_SECRET` in keys dir. (Earlier UAT showed the matching error: `OAuth client not configured for github`.) |
...
- **DeepSeek bridge limit on tool-heavy turns.** Trying to drive *three* MCP `execute_tool` calls inside one Claude Code turn against the DeepSeek anthropic-compatible endpoint returned `API Error: 400 Failed to deserialize the JSON body … messages[1].role: unknown variant 'system'`. Single-tool Claude SDK invocations (covered in the prior UAT report) still work. Looks like a bug in DeepSeek's anthropic-compat shim around multi-step tool-result messages, not a workbench issue. Direct curl through `/mcp` was used as the authoritative validation channel for this matrix.
...
| `uat-dir/.mcp.json` (Bearer API key for the SSO user) | `uat-dir/` | gitignored (`.mcp.json`) |
| Stream-json / text transcripts | `uat-dir/*.transcript.jsonl`, `/tmp/sdk-multi.txt` | gitignored / `/tmp` only |
| Dashboard screenshot | `uat-dir/dashboard-9-live.png` | gitignored (`*.png`) |
...
5. Drop the Bearer into `uat-dir/.mcp.json`.
