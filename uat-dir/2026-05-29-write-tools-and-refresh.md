...
| `bitbucket_list_repos` `{workspace:"amartha"}` | PASS — `Amartha/ng-mis` and siblings |
| `bitbucket_get_repo` `{workspace:"amartha", repoSlug:"ng-mis"}` | PASS |
| `bitbucket_list_prs`  `{workspace:"amartha", repoSlug:"ng-mis"}` | PASS — open PR #5746 + others |
...
| read it back | `google_calendar_get_event` | PASS — `status: confirmed`, htmlLink, summary `"a-workbench UAT test"` |
| change summary | `google_calendar_update_event` | PASS — summary `"a-workbench UAT test (updated)"` |
...
| Step | Tool | Result |
|------|------|--------|
| create doc `"a-workbench UAT doc"` | `google_docs_create` | PASS — `documentId: 1krfmuL0…` |
| read structured | `google_docs_get` | PASS |
| insert text `"UAT body"` at index 1 | `google_docs_batch_update` | PASS |
| read flattened text | `google_docs_get_plaintext` | PASS — `"UAT body\n"` |
...
| Step | Tool | Result |
|------|------|--------|
| create sheet `"a-workbench UAT sheet"` | `google_sheets_create` | PASS — `spreadsheetId: 1yJK…` |
...
| Step | Tool | Result |
|------|------|--------|
| create deck `"a-workbench UAT deck"` | `google_slides_create` | PASS — `presentationId: 1r3s…` |
...
| Step | Tool | Result |
|------|------|--------|
| create folder `"a-workbench UAT folder"` | `google_drive_create_folder` | PASS |
| list folder contents | `google_drive_list` `{folderId}` | PASS |
| upload `a-workbench-uat.txt` with body `"hello uat"` | `google_drive_upload` | PASS |
| download by id | `google_drive_download` | PASS — `"hello uat"` |
...
| Step | Tool | Result |
|------|------|--------|
| create draft to self with subject `"a-workbench UAT"` | `google_gmail_draft` | PASS — `id: r-8090930161310613769`, message stays in `DRAFT` label |
...
- All seven Google plugins: connected + read + write lifecycle (on UAT-created resources) PASS.
